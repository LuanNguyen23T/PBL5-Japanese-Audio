            ...
            <div className="justify-self-start lg:justify-self-center">
              <div className="flex items-center gap-6 text-slate-700">
                <div className="text-center">
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Điểm IRT</p>
          ...
          <div className="grid items-start gap-6 lg:grid-cols-[230px_minmax(0,1fr)] xl:grid-cols-[245px_minmax(0,1fr)]">
                    ...
                    <div className="grid grid-cols-4 gap-y-2.5 gap-x-1.5">
                      {group.questions.map((question) => {
                        ...
                        const toneClass = isCurrentActive
                            ? 'border-2 border-blue-400 bg-white shadow-lg shadow-blue-100/50 scale-110 !font-black !text-blue-700 z-10'
                            : !selectedAnswerId 
            ...
            <main className="min-w-0">
              <div className="space-y-6">
                <div className="rounded-[28px] border border-slate-200 bg-white px-6 py-6 shadow-sm relative overflow-hidden">
                  ...
                  <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
                    ...
                    {(() => {
                      ...
                      if (!selAns) return <div className="px-3 py-1 bg-slate-100 text-slate-500 rounded-lg text-xs font-semibold">Chưa làm</div>
                  ...
                  <div className="rounded-[24px] border border-slate-200 bg-slate-50 px-6 py-6 shadow-inner relative">
                ...
                <div className="space-y-4">
                  {activeQuestion.answers.map((answer, index) => {
                    ...
                    let cardClass = 'border-slate-200 bg-white text-slate-500'
                    let indexClass = 'border-slate-200 text-slate-400'
                    ...
                    return (
                      ...
                      >
                        <span className={['inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full border-4 text-xs font-black relative', indexClass].join(' ')}>
                          {index + 1}
                          {isCorrectAnswer && isSelected && <CheckCircle2 className="absolute -right-2 -top-2 w-5 h-5 text-emerald-500 bg-white rounded-full" />}
                          {!isCorrectAnswer && isSelected && <XCircle className="absolute -right-2 -top-2 w-5 h-5 text-rose-500 bg-white rounded-full" />}
                        </span>
                        ...
                        <div className="flex-1">
                          ...
                          {answer.image_url && (
                            <img
                              ...
                              alt={`Answer ${index + 1}`}
                              className="mt-4 max-h-40 rounded-2xl border border-slate-200 object-contain"
                            />
                          )}
                        </div>
                      </div>
          ...
          <div className="grid items-center gap-4 lg:grid-cols-[160px_minmax(0,1fr)_160px]">
            <div className="justify-self-start">
              <button
                ...
                className="inline-flex h-12 w-24 items-center justify-center rounded-full border-2 border-slate-200 bg-white text-slate-400 transition-colors hover:border-indigo-300 hover:text-indigo-600 disabled:cursor-not-allowed disabled:opacity-50"
              >
