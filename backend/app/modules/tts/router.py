    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=60000.0) as client:
            response = await client.post(
                f"{TTS_SERVICE_URL}/tts/multi-speaker",
                json=tts_payload
            )
            
        if response.status_code != 200:
            logger.error(f"TTS Service Error: {response.text}")
            raise HTTPException(status_code=500, detail="Failed to generate audio from TTS Service.")
        ...
        async def _restart_tts():
            try:
                ...
                subprocess.run(["docker", "restart", "style-bert-vits2"], capture_output=True, timeout=30)
        ...
        raise HTTPException(status_code=503, detail="TTS Service is unavailable. Ensure sbvits2 container is running.")
